...
| <a name="input_auth_mount_tune"></a> [auth\_mount\_tune](#input\_auth\_mount\_tune) | Authentication mount tune | <pre>object({<br>    default_lease_ttl            = optional(string)<br>    max_lease_ttl                = optional(string)<br>    audit_non_hmac_response_keys = optional(list(string))<br>    audit_non_hmac_request_keys  = optional(list(string))<br>    listing_visibility           = optional(string)<br>    passthrough_request_headers  = optional(list(string))<br>    allowed_response_headers     = optional(list(string))<br>    token_type                   = optional(string)<br>  })</pre> | `null` | no |
